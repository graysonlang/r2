// Startup + memory harness for the LOCAL worker path (no libvips). For each
// engine (TS, Wasm) it spawns a fresh worker_threads worker and measures:
//   - spawn → ready          (instantiation lag a respawn re-pays)
//   - first (cold) resize     vs warm resize  (JIT tier-up / lazy-instantiate tax)
//   - RSS + heap + wasm-linear high-water over a long job run  (the creep that
//     motivates a high-water-mark terminate+respawn)
//
...
const tsBundlePath = process.cwd() + '/dist-wasm/separable.bundle.mjs';
const { outputFiles } = await esbuild.build({
  entryPoints: ['src/separable.ts'], bundle: true, format: 'esm', write: false,
});
writeFileSync(tsBundlePath, outputFiles[0].text);

// Ensure the SIMD wasm is built.
if (!existsSync('dist-wasm/resize.mjs')) {
  execFileSync('node', ['scripts/build-wasm.mjs', '--simd'], { stdio: 'ignore' });
}
const wasmPath = process.cwd() + '/dist-wasm/resize.mjs';
...
function spawn(engine) {
  return new Worker(new URL('./startup-worker.mjs', import.meta.url), {
    workerData: { engine, tsBundlePath, wasmPath },
  });
}
...
async function runEngine(engine) {
  ...
  const times = [];
  let rssHi = readyMem.rss, heapHi = readyMem.heapUsed, wasmHi = readyMem.wasmBytes;
  for (let id = 0; id < JOBS; ++id) {
    ...
    wasmHi = Math.max(wasmHi, r.mem.wasmBytes);
  }
  ...
  return { engine, spawnMs, cold, warm, readyRss: readyMem.rss, rssHi, heapHi, wasmHi, mb };
}
...
for (const engine of ['ts', 'wasm']) {
  ...
  if (engine === 'wasm') console.log(`  wasm linear high  ${r.mb(r.wasmHi)} MB  (never shrinks; dies on terminate)`);
  console.log();
}
